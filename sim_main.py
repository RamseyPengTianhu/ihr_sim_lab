...
parser.add_argument("--action_source", type=str, default="dds", 
                   choices=["dds", "file", "trajectory", "policy", "replay","dds_wholebody", "dds_ik", "ros", "ros_ik"], 
                   help="Action source")
...
parser.add_argument("--ik_ee_frame", type=str, default="right_hand", help="IK end-effector frame name")
parser.add_argument("--ik_topic", type=str, default="/ee_target", help="DDS topic for EE pose")
parser.add_argument("--ik_keep_last_sec", type=float, default=0.3, help="hold last IK target seconds")
parser.add_argument("--ik_lowpass", action="store_true", default=True, help="enable low-pass filter for IK q")
parser.add_argument("--ik_lowpass_alpha", type=float, default=0.2, help="low-pass alpha (0~1, small=more smooth)")
parser.add_argument("--ik_upper_body_names", type=str, default="", help="comma-separated 14 upper-body joint names")
parser.add_argument("--dds_domain", type=int, default=0)
parser.add_argument("--dds_nic", type=str, default="")


# ROS-IK topics
parser.add_argument("--ros_ee_topic", type=str, default="/ee_target", help="ROS2 topic for EE pose (PoseStamped)")
parser.add_argument("--ik_lowcmd_topic", type=str, default="/lowcmd", help="ROS2 topic to publish unitree_hg/LowCmd")
parser.add_argument("--lowcmd_pub_rate", type=float, default=60.0, help="LowCmd publish rate (Hz, 0=unlimited)")
...
def main():
    ...
    if not args_cli.replay_data:
        ...
        args_cli.dds_manager = dds_manager
